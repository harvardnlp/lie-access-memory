local MixtureTable, parent
if nn.MixtureTable == nil then
    MixtureTable, parent = torch.class('nn.MixtureTable', 'nn.Module')
else
    MixtureTable = nn.MixtureTable
    parent = nn.Module
end
...
function MixtureTable:updateOutput(input)
   ...
   if self.table or torch.type(expertInputs) == 'table' then
      ...
      if self.batchSize ~= batchSize or prod(self.size) ~= gaterInput:nElement() then
         self.size:resize(expertInput:dim()+1):fill(1)
         if self.dimG > 1 then
            self.size[1] = gaterInput:size(1)
         end
         self.size[self.dim] = gaterInput:size(self.dimG)
         self.output = self.output:type(expertInput:type())
...
function MixtureTable:type(type, tensorCache)
   ...
   self.output:type(type, tensorCache)
