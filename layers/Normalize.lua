...
--[[this version of Normalize computes x/(eps + sum_i x_i^p)^(1/p) for all finite p > 0,
but not for p = inf]]
...
function Normalize:updateGradInput(input, gradOutput)
  ...
  if self.eye:size(2) ~= d then
      self.eye:resize(1, d, d)
      if torch.type(self.eye) == 'torch.CudaTensor' then
          -- cuda doesn't implement eye
          self.eye:double():eye(d):typeAs(input)
      else
          self.eye:eye(d):typeAs(input)
      end
      self.eye = self.eye:view(1, d, d)
  end
